...
const BookingsList = ({
  ...
  const filteredBookings = bookings["hydra:member"].filter(booking => booking.paid > 0 || booking.cards && booking.cards.length > 0).map(booking => ({
  ...
  return <ul className="space-y-2">
      {filteredBookings.map((booking, index) => <li key={index} className="flex items-center gap-2">
...
export function ReservationsTab() {
  ...
  if (isError) {
    return <Card className="p-4 bg-red-50">
  ...
  return <div className="space-y-6">
      <Card className="bg-[#40192C] border-[0.5px] border-[#f3ebad]/30 backdrop-blur-sm hover:shadow-lg transition-all duration-300">
        <div className="p-4">
          <h3 className="font-semibold text-lg text-[#f3ebad] mb-4">Réservations Châtelet</h3>
      ...
      <Card className="bg-[#40192C] border-[0.5px] border-[#f3ebad]/30 backdrop-blur-sm hover:shadow-lg transition-all duration-300">
        <div className="p-4">
          <h3 className="font-semibold text-lg text-[#f3ebad] mb-4">Réservations Pigalle</h3>
