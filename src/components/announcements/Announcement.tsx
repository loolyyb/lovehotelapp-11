...
export function Announcement({
  ...
  useEffect(() => {
    const fetchAdditionalImages = async () => {
      const {
        data,
        error
      } = await supabase.from('announcement_images').select('id, image_url').eq('announcement_id', announcement.id).order('created_at', {
        ascending: true
      });
      if (!error && data) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const {
        error
      } = await supabase.from('announcements').delete().eq('id', announcement.id).eq('user_id', session.user.id);
  ...
  return <div className="backdrop-blur-sm border border-burgundy/20 rounded-lg p-6 space-y-4 shadow-lg transition-colors bg-[#63133b]">
      <div className="flex justify-between items-start">
        <AnnouncementHeader full_name={announcement.full_name} avatar_url={announcement.avatar_url} createdAt={announcement.created_at} />
        {isOwner && <Button variant="ghost" size="icon" onClick={() => setIsDeleteDialogOpen(true)} className="text-zinc-400 hover:text-red-500" disabled={isDeleting}>
            <Trash2 className="h-5 w-5" />
          </Button>}
      </div>
      
      <AnnouncementContent content={announcement.content} imageUrl={announcement.image_url} additionalImages={additionalImages} />
      ...
      <DeleteAnnouncementDialog isOpen={isDeleteDialogOpen} onClose={() => setIsDeleteDialogOpen(false)} onConfirm={handleDelete} />
    </div>;
}